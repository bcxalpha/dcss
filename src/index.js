...
const { runPrometheus, warmupPrometheus, LUA_VERSIONS } = require("./prometheus.js");
...
async function main() {
  ...
  // Warm the Wasmoon factory during startup so the first /obf request does not
  // pay the full WASM/factory initialization cost. Each request still gets a
  // fresh Lua engine, so state is not shared between jobs.
  try {
    await warmupPrometheus();
    console.log("Wasmoon VM warmed up.");
  } catch (error) {
    console.warn("Wasmoon warmup failed; first /obf call will retry:", error.message || error);
  }
